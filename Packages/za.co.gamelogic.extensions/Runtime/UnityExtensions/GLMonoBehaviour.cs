...
namespace Gamelogic.Extensions
{
	...
	public class GLMonoBehaviour : MonoBehaviour
	{
		...
		/// </summary>
		/// <typeparam name="T">The component type on the prefab.</typeparam>
		/// <param name="prefab">The prefab to instantiate.</param>
		/// <param name="root">The parent GameObject to attach the instance to.</param>
		...
		/// <param name="prefab">The prefab to instantiate.</param>
		/// <param name="root">The parent GameObject to attach the instance to.</param>
		/// <param name="localPosition">The local position to set on the instance.</param>
		/// <param name="localRotation">The local rotation to set on the instance.</param>
		/// <returns>The instantiated component.</returns>
		...
		/// <param name="time">The delay in seconds before invoking.</param>
		...
		/// <summary>Invokes the given action after a delay, then repeats it at the given interval.</summary>
		/// <param name="action">The action to invoke.</param>
		/// <param name="time">The initial delay in seconds.</param>
		/// <param name="repeatTime">The interval in seconds between subsequent invocations.</param>
		/// <returns>The started coroutine.</returns>
		[Version(2, 3, 0)]
		...
		/// <summary>Invokes the given action repeatedly, with intervals generated by the given generator.</summary>
		/// <param name="action">The action to invoke.</param>
		/// <param name="repeatTime">A generator that provides each repeat interval in seconds.</param>
		/// <returns>The started coroutine.</returns>
		[Version(2, 3, 0)]
		...
		/// <summary>Tweens a value from <paramref name="start"/> to <paramref name="finish"/> over <paramref name="totalTime"/> seconds.</summary>
		/// <typeparam name="T">The type of the value to tween.</typeparam>
		/// <param name="start">The starting value.</param>
		/// <param name="finish">The ending value.</param>
		/// <param name="totalTime">The total duration of the tween in seconds.</param>
		/// <param name="lerp">A function to interpolate between two values.</param>
		/// <param name="action">A callback invoked each frame with the current interpolated value.</param>
		/// <param name="deltaTime">A function that returns the delta time for each frame.</param>
		/// <returns>The started coroutine.</returns>
		[Version(2, 3, 0)]
		...
		/// <inheritdoc cref="Tween{T}(T,T,float,Func{T,T,float,T},Action{T},Func{float})"/>
		[Version(2, 3, 0)]
		public Coroutine Tween<T>(
			T start,
			T finish,
	...
	public static class MonoBehaviourExtensions
	{
		...
		/// <summary>
		/// Clones a MonoBehaviour using <see cref="Object.Instantiate{T}(T)"/>.
		/// </summary>
		/// <typeparam name="T">The MonoBehaviour type to clone.</typeparam>
		...
		/// <typeparam name="T">The MonoBehaviour type to clone.</typeparam>
		/// <param name="obj">The object to clone.</param>
		/// <param name="count">The number of clones to create.</param>
		/// <returns>A list of cloned instances.</returns>
		...
		#region Tweening
		/// <inheritdoc cref="GLMonoBehaviour.Tween{T}(T,T,float,Func{T,T,float,T},Action{T})"/>
		...
		/// <inheritdoc cref="GLMonoBehaviour.Tween{T}(T,T,float,Func{T,T,float,T},Action{T},Func{float})"/>
		...
		/// <summary>Finds a direct child of the component's transform with the given name.</summary>
		/// <param name="component">The component whose transform to search.</param>
		/// <param name="childName">The name of the child to find.</param>
		/// <returns>The child's GameObject.</returns>
		...
		/// <summary>Finds a child of the component's transform with the given name, optionally searching recursively.</summary>
		/// <param name="component">The component whose transform to search.</param>
		/// <param name="childName">The name of the child to find.</param>
		/// <param name="recursive">If <see langword="true"/>, searches only the direct children; if <see langword="false"/>, searches the entire hierarchy.</param>
		/// <returns>The matching child's GameObject, or <see langword="null"/> if not found.</returns>
		...
		/// <returns>The attached component cast to <typeparamref name="TInterface"/>, or <see langword="null"/> if not found.</returns>
